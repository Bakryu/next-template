  ...
  return (
    <Section id="promotions" variant="muted">
      <Container>
        ...
        {/* Mobile carousel */}
        <MobileCarousel slideWidth="w-[82vw]" className="-mx-4 sm:hidden">
          {items.map((item, i) => (
            <div
              key={i}
              className="group border-border/40 bg-card relative flex h-full flex-col rounded-2xl border px-7 pt-10 pb-7 shadow-sm"
            >
              {item.badge && (
                <Badge variant="secondary" size="sm" className="absolute top-4 left-7">
                  {item.badge === 'new'
                    ? t('badgeNew')
                    : item.badge === 'hot'
                      ? t('badgeHot')
        ...
        <div className="hidden gap-6 sm:grid sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item, i) => (
            <motion.div
              ...
              className="group border-border/40 bg-card hover:border-secondary/30 relative flex flex-col rounded-2xl border px-7 pt-10 pb-7 shadow-sm transition-all duration-300 hover:shadow-md"
            >
              {item.badge && (
                <Badge variant="secondary" size="sm" className="absolute top-4 left-7">
                  {item.badge === 'new'
